...
class SingleWeekdayTimeslot(OpeningTimeslot):
    def _init_(self, weekday, time_from, time_to, is_positive):
...
class SingleMonthTimeslot(OpeningTimeslot):
    def _init_(self, month, time_from, time_to, is_positive):
        self.month = month
        self.time_from = time_from
        self.time_to = time_to
        self.is_positive = is_positive
...
class SingleMonthdayTimeslot(OpeningTimeslot):
    def _init_(self, month, monthday, time_from, time_to, is_positive):
        self.month = month
                    ...
                    if ":" not in part:
                        if "-" not in part:
                            ...
                            for current_day in range(int(day_from), int(day_to) + 1):
                                timeslots.append(SingleMonthdayTimeslot(month, str(current_day), "00:00", "24:00", is_positive))
                    else:
                        ...
                        if subpart_count > 2:
                            ...
                            for current_day in range(int(day_from), int(day_to) + 1):
                                timeslots.append(SingleMonthdayTimeslot(month, str(current_day), time_from, time_to, is_positive))
                        else:
...
result = OSM_Parser.get_timeslot("24/7; Mar 15:00-17:00; sunrise-sunset; Aug 31 off; Jan 17-23 09:33-17:45; Mo 10:00-20:00; We 08:00-02:00; Tu-Fr 08:00-12:00,12:30-15:00 off")
for ts in result:
    print(ts)
